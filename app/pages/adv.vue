<template>
	<div>
		...
		>
			...
			>
				...
				<template #content>
					<div class="flex justify-between items-center gap-4">
						<USelectMenu v-model="activeTitle" :items="advTitles" />
								...
								<template #content>
									<div class="p-4 flex flex-col gap-4">
										<UFormField label="Text Speed">
											<UInputNumber v-model="textSpeed" :min="1" :max="10" />
										</UFormField>
			...
			>
				<p :class="textSize">
					<span
						...
						:style="{
							maskImage: cancelAnimation
								? ''
								: 'linear-gradient(90deg, #000 0%, #000 33.33%, transparent 66.67%, transparent 100%)',
							animation: cancelAnimation
								? ''
								: `reveal ${(500 / textSpeed) * 3}ms ease-in forwards`,
			...
			<div class="text-sm text-muted flex justify-center items-center mt-4">
				<UButton
					variant="ghost"
					color="neutral"
					size="sm"
		...
		<UModal v-model:open="synchronizeModalOpen" title="是否同步数据">
			<template #body>
			...
			<template #footer>
				...
				<UButton class="grow justify-center" @click="confirmSynchronization">
					确认
				</UButton>
			</template>
		</UModal>
...
const advTitles = computed(() =>
	advTitleData.value?.map(item => ({ label: item.title, id: item.stem })),
)
const activeTitle = computed({
	get() {
		return (
			advTitles.value?.find(item => item.id === advState.value.active) ??
			advTitles.value?.[0]
		)
	},
	set(newVal) {
...
const { data: advData, refresh: refreshAdvData } = await useAsyncData(
	'adv',
	() => {
		return queryCollection('adv')
			.where('stem', '=', advState.value.active)
			.first()
	},
	{
		server: false,
...
const progress = computed({
	...
	set(newValue) {
		if (advData.value) {
			...
			advState.value.progress[advData.value.stem] =
				newValue % advData.value.content.length
			advState.value.updatedAt = Date.now()
			synchronizationState.executed = false
		}
	},
