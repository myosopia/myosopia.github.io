<template>
  <div>
      ...
      >
        <UCard>
          ...
          <div class="space-y-2">
            ...
            >
              <li
                v-for="(def, didx) in item.definition"
                :key="didx"
              >
                {{ def }}
              </li>
    ...
    >
      <p>{{ t('startSearching') || 'Start typing to search the dictionary...' }}</p>
    </div>
  </div>
</template>
...
const { data: dictionaryList } = await useAsyncData('dictionary-list', async () => {
...
const { data: dictionaryData, refresh } = await useAsyncData('dictionary', async () => {
  ...
  for (const dictEntry of dictionaryData.value || []) {
    for (const [word, definition] of Object.entries(dictEntry.body!)) {
      ...
      if (word.toLowerCase().includes(query)) {
        matches.push({ word: word, definition: definition, dictionary: dictEntry.title })
      }
    }
  }
